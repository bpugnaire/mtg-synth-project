...
export function create(tracks: Track[], beatNotifier: BeatNotifier, loopLength: number, bpm: number): Tone.Sequence {
  const loop = new Tone.Sequence(
    loopProcessor(tracks, beatNotifier),
    new Array(loopLength).fill(0).map((_, i) => i),
    /*loopLength.toString().concat(*/"4n")
  ;
